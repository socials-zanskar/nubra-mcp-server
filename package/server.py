...
def register_tools(mcp: FastMCP, service: NubraService) -> None:
    ...
    portfolio.register(mcp, service)
    risk.register(mcp, service)
    screener.register(mcp, service)
    backtest.register(mcp, service)
    journal.register(mcp, service)
...
def create_app(settings: Settings, mcp: FastMCP) -> FastAPI:
    ...
    def health() -> dict[str, Any]:
        return {
            ...
            "trading_mode": "uat_only" if settings.environment == "UAT" else "read_only",
            "trading_enabled": settings.environment == "UAT",
            "mcp_path": settings.mcp_path,
        }
